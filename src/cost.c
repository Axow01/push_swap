...
int	ft_cost(t_pile *node)
{
	...
	if (future_node->index + 1 > ft_get_largest(get_data()->b) / 2)
		future_node->rrotate = ft_list_lenght(get_data()->b) - future_node->index;
	else
	...
	return (future_node->rrotate + future_node->rotate + node->rotate + node->rrotate + 1 - ft_cost_optimise(node, future_node));
}
